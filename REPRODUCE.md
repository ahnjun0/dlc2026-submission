...
집계          **적응형 캐스케이드**(exp-101) · 순서 무관 동률 정책 다수결
              ① STAR 32표의 최다 득표 ≥30  →  STAR 단독으로 확정
              ② 아니면 STAR + LONGCOT = 64표 풀링
              ③ 그 1·2위 격차 ≤5  →  STAR-OLY 32표 증원 (96표)
```
...
구성만 바꾸면 **STAR-OLY +3.00 > STAR 재뽑기 +2.00 > LONGCOT −2.00** 이다.
즉 3단계는 «표 증량» 이 아니라 **«접전 구간에서 STAR 계열로의 조건부 재가중»** 이다.
2단계는 «다른 **체제**»(LONGCOT)를, 3단계는 «같은 체제 · 다른 **데이터**»(STAR-OLY)를 요구한다.
...
표준 구성   confirm 30 · margin 5           **약 12.3시간**  (PRO 5000 급)
            1단계 STAR 2,000문항 1시간 45분 · 2단계 LONGCOT 0.68분/문항 · 3단계 ~35분
...
> **STAR 데이터와 혼합하지 않는다.** 1:4 로 희석했더니 φ 가 0.726 → 0.878 로 올라
...
**실측 소요**
```
LB 831 · 전량 풀링 (4090)      6시간 39분   STAR 31분 + LONGCOT 6h08m + 제출 19초
test 2,000 · 캐스케이드        **4090 ~17시간 · RTX PRO 6000 ~9.5시간**
```
> ⚠ **캐스케이드의 시간 절감은 작다.** STAR 가 확신하는 **쉬운** 문항을 생략하므로
> LONGCOT에 남는 문항이 전부 비싸다 — 선별분 **0.916 분/문항** vs 전량 평균 0.443 (**2.07배**).
...
7. **HF 어댑터 경로** — STAR 는 `exp-004-v2-adapter/` 이지 `archive/ckpt/` 가 아니다.
