...
**그리고 실제로 누출이 없었음을 감사했습니다.** 외부 풀에서 만든 두 학습 데이터를
리더보드 **원본 1,000행 전체**(필터본 831행이 아니라)와 정규화 대조했습니다.

| 학습 데이터 | 행 수 | 운영진 제외 169문항과 일치 | LB 원본 1,000 전체와 일치 |
|---|---|---|---|
| `sft_contest.jsonl` (STAR-OLY) | 23,593 | **0건** | **0건** |
| `moonshot_r1.jsonl` (LONGCOT) | 25,953 | **0건** | **0건** |

> 빌드 시점의 dedup 은 필터본 831행을 기준으로 했습니다. 사후 감사에서 누출이 0건임을
> 확인했고, **절차 자체는 원본 1,000행까지 대조하도록 교정**했습니다
> (`src/data/build_contest_pool.py`).
